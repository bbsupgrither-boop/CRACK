...
import { ArrowLeft, Plus, ChevronDown, Edit, Trash2, X, Upload, Home, Users, Zap, Trophy, CheckSquare, ShoppingBag, Gamepad2, Box, RotateCcw } from './Icons';
...
export function WorkersManagement({ onBack, onNavigateToSection }: WorkersManagementProps) {
  ...
  const roles = ['Воркер', 'Тимлид', 'мл. Админ', 'ст. Админ'];
    ...
    <div className="min-h-screen bg-background">
      ...
      {showAddWorker && (
        <WorkerModal
          ...
          onSave={(workerData) => {
            // Логика добавления нового воркера
      ...
      {showEditWorker && selectedWorker && (
        <WorkerModal
          ...
          onSave={(workerData) => {
            // Логика обновления воркера
            console.log('Updating worker:', workerData);
            setShowEditWorker(false);
            setSelectedWorker(null);
          }}
          onFire={(reason, comment, file) => {
            // Логика увольнения воркера
            console.log('Firing worker:', selectedWorker?.id, reason, comment, file);
            setShowEditWorker(false);
            setSelectedWorker(null);
          }}
          teamLeads={teamLeads}
    ...
    <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="bg-background border-none max-w-sm p-0 [&>button]:hidden max-h-[90vh] overflow-hidden rounded-3xl">
        <DialogTitle className="text-lg font-medium text-foreground text-center">
          {isEdit ? 'Анкета' : 'Добавить воркера'}
        </DialogTitle>
        <DialogDescription className="sr-only">
          {isEdit ? 'Форма для редактирования данных сотрудника' : 'Форма для добавления нового сотрудника'}
        </DialogDescription>
        <div className="p-6">
          <DialogHeader>
            <div className="flex items-center justify-between mb-4">
